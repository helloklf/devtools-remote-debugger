/**
 * 加载lzma压缩的资源包并解压写入Cache Storage
 * 大幅的减少了网络请求数量
 */
...
self.addEventListener('install', () => {
  ...
  const now = Date.now();
  console.log('service worker - get devtools.lzma')
  fetch('./devtools_package.lzma')
    .then(r => r.arrayBuffer())
    .then(r => LZMA.decompress(new Uint8Array(r)))
    .then(async (data) => {
      console.log(
        'service worker - download succeed! (' + (Date.now() - now) + 'ms)'
      );
      // 不知道为何，使用lzma-c.js压缩的数据，解压后直接是文本
      // 用c++版lzma压缩的数据解压后是Array<Int>，这里做个兼容处理
      ...
      let offset = 0;
      // 取前8位，得到文件表长度
      const mapHeadLen = parseInt(data.substring(offset, 8).trim());
      offset += 8;
      // 截取文件表，格式如 [["./.editorconfig",147],["./Images/accelerometer-back.svg",756],["./Images/accelerometer-bottom.png",189] ... 标识每个文件的长度，分割文件时按这个信息处理
      const fileInfo = JSON.parse(data.substring(offset, offset + mapHeadLen));
      offset += mapHeadLen;

      const cache = await caches.open(storageName);

      for (const [filePath, fileLength] of fileInfo) {
        const fileData = data.substring(offset, offset + fileLength);
        offset += fileLength;
        ...
        const response = new Response(fileData, {
          headers: { 'Content-Type': contentType || 'text/plain', 'Content-Length': fileData.length }
        });
        await cache.put(filePath, response);
      }

      console.log('service worker - preload completely! (' + (Date.now() - now) + 'ms)');
    });
});
